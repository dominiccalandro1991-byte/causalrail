...
apiRouter.get(
  ...
  wrap(async (req, res) => {
    ...
    const runs = await query(
      `select * from build_runs where user_id = $1 order by created_at desc limit 80`,
      [userId],
    );
    ...
    res.json(assembleDashboard(userId, runs, analyses));
  }),
);
...
apiRouter.get(
  ...
  wrap(async (req, res) => {
    ...
    const siblings = run.fingerprint
      ...
      : [];
    res.json({ run, analysis: analyses[0] ?? null, siblings, live: true });
  }),
);
